...
"""
        ...
        has_copied_from = (
            re.search("^#\s+Copied from", obj, flags=re.MULTILINE) is not None
        )
        if (
            add_copied_from
...
_re_checkpoint_for_doc = re.compile(
    "^_CHECKPOINT_FOR_DOC\s+=\s+(\S*)\s*$", flags=re.MULTILINE
)
...
def retrieve_info_for_model(model_type, frameworks: Optional[List[str]] = None):
    ...
    archive_map = auto_module.configuration_auto.CONFIG_ARCHIVE_MAP_MAPPING_NAMES.get(
        model_type, None
    )
...
def clean_frameworks_in_init(
    ...
    while idx < len(lines):
        ...
        # Otherwise we keep the line, except if it's a tokenizer import and we don't want to keep it.
            ...
            re.search('^\s*"(tokenization|processing|feature_extraction)', lines[idx])
            is None
            and re.search(
                "^\s*from .(tokenization|processing|feature_extraction)", lines[idx]
            )
            is None
        ):
        ...
        for pattern in new_patterns:
            ...
            if "pretrained_archive_map" in pattern:
                old_model_line = old_model_line.replace(
                    ...
                    f"{old_model_patterns.model_upper_cased}_PRETRAINED_CONFIG_ARCHIVE_MAP",
                )
                new_model_line = new_model_line.replace(
                    "{pretrained_archive_map}",
                    f"{new_model_patterns.model_upper_cased}_PRETRAINED_CONFIG_ARCHIVE_MAP",
...
def duplicate_doc_file(
    ...
    content = re.sub(
        "<!--\s*Copyright (\d+)\s", f"<!--Copyright {CURRENT_YEAR} ", content
    )
    if frameworks is None:
        frameworks = get_default_frameworks()
    if dest_file is None:
...
def create_new_model_like(
    ...
    doc_file = (
        ...
        / "en"
        / "model_doc"
        / f"{old_model_patterns.model_type}.mdx"
    )
    duplicate_doc_file(
...
def get_user_input():
    ...
    old_processing_classes = [
        c
        for c in [old_feature_extractor_class, old_tokenizer_class, old_processor_class]
        if c is not None
    ]
