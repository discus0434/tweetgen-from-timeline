...
def rename_keys(state_dict, encoder_only=False):
    ...
    for key, value in state_dict.items():
        ...
        if "segformer.encoder.layer_norm" in key:
            ...
            idx = key[
                key.find("segformer.encoder.layer_norm")
                + len("segformer.encoder.layer_norm")
            ]
    ...
    for i in range(config.num_encoder_blocks):
        for j in range(config.depths[i]):
            ...
            kv_weight = state_dict.pop(
                f"segformer.encoder.block.{i}.{j}.attention.self.kv.weight"
            )
            kv_bias = state_dict.pop(
                f"segformer.encoder.block.{i}.{j}.attention.self.kv.bias"
            )
            # next, add keys and values (in that order) to the state dict
            state_dict[
                f"segformer.encoder.block.{i}.{j}.attention.self.key.weight"
            ] = kv_weight[: config.hidden_sizes[i], :]
            state_dict[
                f"segformer.encoder.block.{i}.{j}.attention.self.key.bias"
            ] = kv_bias[: config.hidden_sizes[i]]
            state_dict[
                f"segformer.encoder.block.{i}.{j}.attention.self.value.weight"
            ] = kv_weight[config.hidden_sizes[i] :, :]
            state_dict[
                f"segformer.encoder.block.{i}.{j}.attention.self.value.bias"
            ] = kv_bias[config.hidden_sizes[i] :]
    ...
    # Cityscapes checkpoints
        ...
        expected_slice = torch.tensor(
            [
                ...
                [
                    [1.0491, 0.8289, 1.0310],
                    [1.1044, 0.5219, 0.8055],
                    [1.0899, 0.6926, 0.5590],
                ],
            ]
        )
