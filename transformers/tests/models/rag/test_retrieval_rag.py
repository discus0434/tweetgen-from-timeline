...
class RagRetrieverTest(TestCase):
    ...
    def get_dummy_legacy_index_retriever(self):
        ...
        passages = {
            sample["id"]: [sample["text"], sample["title"]] for sample in dataset
        }
    ...
    def test_legacy_index_retriever_retrieve(self):
        ...
        self.assertEqual(
            doc_dicts[0]["text"][0], "bar"
        )  # max inner product is reached with second doc
        self.assertEqual(
            doc_dicts[1]["text"][0], "foo"
        )  # max inner product is reached with first doc
        self.assertListEqual(doc_ids.tolist(), [[1], [0]])
    ...
    def test_custom_hf_index_end2end_retriever_call(self):
        ...
        self.assertEqual(
            all(
                k in out for k in ("tokenized_doc_ids", "tokenized_doc_attention_mask")
            ),
            True,
