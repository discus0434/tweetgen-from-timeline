...
class TFLogitsProcessorTest(unittest.TestCase):
    ...
    def test_min_length_dist_processor(self, use_xla):
        ...
        min_dist_processor = TFMinLengthLogitsProcessor(
        ...
        self.assertListEqual(
            scores_before_min_length[:, eos_token_id].numpy().tolist(),
            4 * [-float("inf")],
        )
        ...
        scores_before_min_length = min_dist_processor(input_ids, scores, cur_len)
        self.assertFalse(
            tf.math.reduce_any(tf.math.is_inf(scores_before_min_length)).numpy()
        )
    ...
    def test_temperature_dist_warper(self, use_xla):
        ...
        if use_xla:
            temp_dist_warper_sharper = tf.function(
                temp_dist_warper_sharper, jit_compile=True
            )
            temp_dist_warper_smoother = tf.function(
                temp_dist_warper_smoother, jit_compile=True
            )

        warped_prob_sharp = tf.nn.softmax(
            temp_dist_warper_sharper(input_ids, tf.identity(scores), cur_len), axis=-1
        )
        warped_prob_smooth = tf.nn.softmax(
            temp_dist_warper_smoother(input_ids, tf.identity(scores), cur_len), axis=-1
        )
        ...
        self.assertLess(
            tf.math.reduce_max(probs[1, :]), tf.math.reduce_max(warped_prob_sharp[1, :])
        )
        self.assertGreater(
            tf.math.reduce_min(probs[1, :]), tf.math.reduce_min(warped_prob_sharp[1, :])
        )

        # smooth peaks get lower, valleys get higher
        self.assertGreater(
            tf.math.reduce_max(probs[1, :]),
            tf.math.reduce_max(warped_prob_smooth[1, :]),
        )
        self.assertLess(
            tf.math.reduce_min(probs[1, :]),
            tf.math.reduce_min(warped_prob_smooth[1, :]),
        )
    ...
    def test_top_k_dist_warper(self, use_xla):
        ...
        self.assertListEqual(
            tf.math.is_inf(scores[0]).numpy().tolist(), 7 * [True] + 3 * [False]
        )
        self.assertListEqual(
            tf.math.is_inf(scores[1]).numpy().tolist(),
        ...
        top_k_warp_safety_check = TFTopKLogitsWarper(
            top_k=1, filter_value=0.0, min_tokens_to_keep=3
        )
        if use_xla:
            top_k_warp_safety_check = tf.function(
                top_k_warp_safety_check, jit_compile=True
            )

        scores = top_k_warp_safety_check(input_ids, logits, cur_len)
        # uniform dist is not changed
        self.assertListEqual(
            tf.math.reduce_sum(tf.where(scores == 0.0, 1, 0), axis=-1).numpy().tolist(),
            [0, 0],
        )
    ...
    def test_top_p_dist_warper(self, use_xla):
        ...
        dist = np.log(
            np.array([[0.3, 0.1, 0.1, 0.5], [0.15, 0.3, 0.3, 0.25]], dtype=np.float32)
        )
        ...
        # exp (-inf) => 0
        EXPECTED_FILTERED_DIST = tf.constant(
            [[0.3, 0.0, 0.0, 0.5], [0.0, 0.3, 0.3, 0.25]], dtype=tf.float32
        )
        ...
        # 2.
        self.assertListEqual(
            tf.math.reduce_sum(tf.where(filtered_dist != 0.0, 1, 0), axis=-1)
            .numpy()
            .tolist(),
            [3, 2],
        )
    ...
    def test_no_repeat_ngram_dist_processor(self):
        ...
        filtered_scores_2_gram = no_repeat_proc_2_gram(
            input_ids, tf.identity(scores), cur_len
        )
        ...
        self.assertListEqual(
            tf.math.is_inf(filtered_scores_2_gram).numpy().tolist(),
            [[False, True, True], [True, False, False]],
        )

        # 3-gram would forbid no token at 1st batch and 1st token (0) at 2nd batch
        self.assertListEqual(
            tf.math.is_inf(filtered_scores_3_gram).numpy().tolist(),
    ...
    def test_no_bad_words_dist_processor(self, use_xla):
        ...
        no_bad_words_dist_proc = TFNoBadWordsLogitsProcessor(
            bad_words_ids=bad_word_tokens, eos_token_id=eos_token_id
        )
        if use_xla:
            no_bad_words_dist_proc = tf.function(
                no_bad_words_dist_proc, jit_compile=True
            )

        filtered_scores = no_bad_words_dist_proc(
            input_ids, tf.identity(scores), cur_len
        )
    ...
    def test_forced_bos_token_logits_processor(self, use_xla):
        ...
        self.assertTrue(
            tf.math.reduce_all(
                tf.math.is_inf(scores[:, bos_token_id + 1 :])
                & (scores[:, bos_token_id + 1 :] < 0)
            )
        )
        self.assertListEqual(
            scores[:, bos_token_id].numpy().tolist(), 4 * [0]
    ...
    def test_forced_eos_token_logits_processor(self, use_xla):
        ...
        logits_processor = TFForcedEOSTokenLogitsProcessor(
        ...
        self.assertTrue(
            tf.math.reduce_all(
                tf.math.is_inf(scores[:, eos_token_id + 1 :])
    ...
    def test_processor_list(self, use_xla):
        ...
        min_dist_proc = TFMinLengthLogitsProcessor(
            min_length=10, eos_token_id=eos_token_id
        )
        ...
        self.assertListEqual(
            input_ids.numpy().tolist(), input_ids_comp.numpy().tolist()
        )
