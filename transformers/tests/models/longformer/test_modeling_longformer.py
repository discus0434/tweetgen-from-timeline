...
class LongformerModelTester:
    ...
    def create_and_check_attention_mask_determinism(
        ...
        self.parent.assertTrue(
            torch.allclose(
                output_with_mask[0, 0, :5], output_without_mask[0, 0, :5], atol=1e-4
            )
        )
    ...
    def prepare_config_and_inputs_for_question_answering(self):
        ...
        input_ids[input_ids == config.sep_token_id] = torch.randint(
...
class LongformerModelIntegrationTest(unittest.TestCase):
    ...
    def test_diagonalize(self):
        ...
        self.assertTrue(
            ...
            == chunked_hidden_states.shape[-1] + window_overlap_size - 1
        )
    ...
    def test_pad_and_transpose_last_two_dims(self):
        ...
        self.assertTrue(
            torch.allclose(
                expected_added_dim, padded_hidden_states[0, -1, :], atol=1e-6
            )
        )
        self.assertTrue(
            torch.allclose(
    ...
    def test_chunk(self):
        ...
        self.assertTrue(
            torch.allclose(
                chunked_hidden_states[0, 0, :, 0], expected_slice_along_chunk, atol=1e-3
            )
        )
    ...
    def test_layer_local_attn(self):
        ...
        attention_mask = torch.zeros(
            (batch_size, seq_length), dtype=torch.float32, device=torch_device
        )
    ...
    def test_layer_global_attn(self):
        ...
        hidden_states = torch.cat(
            [self._get_hidden_states(), self._get_hidden_states() - 0.5], dim=0
        )
        batch_size, seq_length, hidden_size = hidden_states.size()
        attention_mask = torch.zeros(
            (batch_size, seq_length), dtype=torch.float32, device=torch_device
    ...
    def test_layer_attn_probs(self):
        ...
        self.assertTrue(
            torch.all(torch.abs(global_attentions[0, :, :2, :].sum(dim=-1) - 1) < 1e-6)
        )
        self.assertTrue(
            torch.all(torch.abs(global_attentions[1, :, :1, :].sum(dim=-1) - 1) < 1e-6)
        )
    ...
    def test_inference_no_head(self):
        ...
        expected_output_slice = torch.tensor(
            [0.0549, 0.1087, -0.1119, -0.0368, 0.0250], device=torch_device
        )
        self.assertTrue(
            torch.allclose(output[0, 0, -5:], expected_output_slice, atol=1e-4)
        )
        self.assertTrue(
            torch.allclose(
                output_without_mask[0, 0, -5:], expected_output_slice, atol=1e-4
            )
        )
    ...
    def test_inference_masked_lm_long(self):
        ...
        self.assertTrue(
            torch.allclose(
                prediction_scores.sum(), expected_prediction_scores_sum, atol=1e-4
            )
        )
        self.assertTrue(
            torch.allclose(
                prediction_scores.mean(), expected_prediction_scores_mean, atol=1e-4
            )
        )
