...
def convert_bigbird_pegasus(
    tf_weights: dict, config_update: dict
    ...
    decoder_weights = {
        k: tf_weights[k] for k in tf_weights if k.startswith("pegasus/decoder")
    }
    remaining_weights = {
        k: tf_weights[k] for k in tf_weights if not k.startswith("pegasus/decoder")
    }
    ...
    for k, v in tqdm(decoder_weights.items(), "tf -> hf conversion"):
        ...
        assert (
            v.shape == state_dict[new_k].shape
        ), f"{new_k}, {k}, {v.shape}, {state_dict[new_k].shape}"
    ...
    mapping["model.encoder.embed_positions.weight"] = mapping[
        "model.embed_positions.weight"
    ]
    mapping["model.decoder.embed_positions.weight"] = mapping.pop(
        "model.embed_positions.weight"
    )
...
def convert_bigbird_pegasus_ckpt_to_pytorch(
    ckpt_path: str, save_dir: str, config_update: dict
):
...
if __name__ == "__main__":
    ...
    config_update = {}
    convert_bigbird_pegasus_ckpt_to_pytorch(
