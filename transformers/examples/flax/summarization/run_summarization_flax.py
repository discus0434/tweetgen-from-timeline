...
def main():
    ...
    for epoch in epochs:
        ...
        if data_args.predict_with_generate:
            ...
            rouge_desc = " ".join(
                [f"Eval {key}: {value} |" for key, value in rouge_metrics.items()]
            )
    ...
    if training_args.do_predict:
        ...
        pred_loader = data_loader(
            input_rng, predict_dataset, eval_batch_size, drop_last=False
        )
        ...
        if data_args.predict_with_generate:
            ...
            rouge_desc = " ".join(
                [f"Predict {key}: {value} |" for key, value in rouge_metrics.items()]
            )
