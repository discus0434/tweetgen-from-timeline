...
class BeamSearchTester:
    ...
    def prepare_inputs(self):
        ...
        next_tokens = ids_tensor(
            (self.batch_size, 2 * self.num_beams), self.vocab_size
        ).to(torch_device)
        next_indices = ids_tensor(
            (self.batch_size, 2 * self.num_beams), self.num_beams
        ).to(torch_device)
        next_scores, _ = (
            -floats_tensor((self.batch_size, 2 * self.num_beams)).to(torch_device)
    ...
    def check_beam_hypotheses(self, input_ids, *args):
        ...
        self.parent.assertAlmostEqual(
            beam_hyp.worst_score,
            -9.0 / (self.sequence_length**beam_hyp.length_penalty),
        )
    ...
    def check_beam_scorer_update(
        self, input_ids, next_tokens, next_indices, next_scores
    ):
        ...
        beam_indices = torch.zeros_like(input_ids) + torch.arange(
        ...
        def cut_expected_tensor(tensor):
            return torch.cat(
                [tensor[:, :1], tensor[:, 2 : self.num_beams + 1]], dim=1
            ).flatten()
        ...
        expected_output_indices = (
            ...
            + (
                torch.arange(self.num_beams * self.batch_size, device=torch_device)
                // self.num_beams
            )
            * self.num_beams
        )
        ...
        self.parent.assertTrue(
            torch.allclose(expected_output_scores, output_scores, atol=1e-3)
        )
    ...
    def check_beam_scores_finalize(
        ...
        self.parent.assertListEqual(
            list(sequences.shape), [self.batch_size, max_length]
        )
        ...
        self.parent.assertListEqual(
            list(sequences.shape), [self.num_beams * self.batch_size, max_length]
        )
        self.parent.assertListEqual(
            list(sequence_scores.shape), [self.num_beams * self.batch_size]
        )
...
class ConstrainedBeamSearchTester:
    ...
    def prepare_inputs(self):
        ...
        scores_for_all_vocab, _ = (
            -floats_tensor((self.batch_size * self.num_beams, self.vocab_size)).to(
                torch_device
            )
        ).sort(descending=True)
    ...
    def check_beam_hypotheses(self, input_ids, *args):
        ...
        self.parent.assertEqual(
            len(constrained_beam_scorer._beam_hyps), self.batch_size
        )
    ...
    def check_constrained_beam_scorer_update(
        ...
        for batch_idx in range(self.batch_size):
            ...
            self.parent.assertListEqual(
                input_ids[correct_idx].tolist(),
                constrained_beam_scorer._beam_hyps[batch_idx].beams[0][1].tolist(),
            )
    ...
    def check_constrained_beam_scorer_finalize(
        ...
        for output, constraint in [(s, c) for s in sequences for c in constraints]:
            ...
            if isinstance(forced_token_ids[0], list):
                ...
                self.parent.assertEqual(
                    self._check_sequence_inside_sequence(output, forced_token_ids), True
                )
...
class ConstrainedBeamSearchTest(unittest.TestCase):
    ...
    def test_constrained_beam_scorer_update(self):
        inputs = self.constrained_beam_search_tester.prepare_inputs()
        self.constrained_beam_search_tester.check_constrained_beam_scorer_update(
            *inputs
        )

    def test_constrained_beam_scorer_finalize(self):
        inputs = self.constrained_beam_search_tester.prepare_inputs()
        self.constrained_beam_search_tester.check_constrained_beam_scorer_finalize(
            *inputs
        )
