...
def train_epoch(
    data_loader, discriminator, optimizer, epoch=0, log_interval=10, device="cpu"
):
    samples_so_far = 0
    ...
    with torch.no_grad():
        for input_t, target_t in data_loader:
            ...
            test_loss += nn.functional.nll_loss(
                output_t, target_t, reduction="sum"
            ).item()
...
def train_discriminator(
    ...
    for epoch in range(epochs):
        ...
        print("\nExample prediction")
        predict(
            example_sentence, discriminator, idx2class, cached=cached, device=device
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--epochs", type=int, default=10, metavar="N", help="Number of training epochs"
    )
    parser.add_argument(
        "--batch_size",
        type=int,
