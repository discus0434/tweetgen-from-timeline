...
def pack_data_dir(tok, data_dir: Path, max_tokens, save_path):
    ...
    for split in ["train"]:
        ...
        print(
            f"packed {split} split from {len(src_docs)} examples -> {len(packed_src)}."
        )
...
def packer_cli():
    ...
    parser.add_argument(
        "--tok_name", type=str, help="like facebook/bart-large-cnn,t5-base, etc."
    )
    ...
    return pack_data_dir(
        tokenizer, Path(args.data_dir), args.max_seq_len, args.save_path
    )
