    ...
    for i in range(feature_extract_tester.batch_size):
        if equal_resolution:
            ...
            if getattr(feature_extract_tester, "size_divisor", None):
                ...
                min_resolution = max(
                    feature_extract_tester.size_divisor, min_resolution
                )
            width, height = np.random.choice(
                np.arange(min_resolution, feature_extract_tester.max_resolution), 2
            )
        image_inputs.append(
            np.random.randint(
                255,
...
def prepare_video(
    feature_extract_tester, width=10, height=10, numpify=False, torchify=False
):
...
class FeatureExtractorPushToHubTester(unittest.TestCase):
    ...
    def tearDownClass(cls):
        ...
        try:
            delete_repo(
                token=cls._token, repo_id="valid_org/test-feature-extractor-org"
            )
        except HTTPError:
            pass
    ...
    def test_push_to_hub(self):
        ...
        feature_extractor.push_to_hub(
            "test-feature-extractor", use_auth_token=self._token
        )
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            feature_extractor.save_pretrained(
                tmp_dir,
                repo_id="test-feature-extractor",
                push_to_hub=True,
    ...
    def test_push_to_hub_in_organization(self):
        ...
        feature_extractor.push_to_hub(
            "valid_org/test-feature-extractor", use_auth_token=self._token
        )
    ...
    def test_push_to_hub_dynamic_feature_extractor(self):
        ...
        feature_extractor.push_to_hub(
            "test-dynamic-feature-extractor", use_auth_token=self._token
        )
