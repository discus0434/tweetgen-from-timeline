...
def _convert_past_list_to_tuple(past_key_values):
    ...
    for idx in range(count_n):
        ...
        temp_result = tuple(
            past_key_values[real_idx : real_idx + count_of_each_inner_tuple]
        )
        results += ((temp_result),)
...
def _create_traced_decoder(
    ...
    if past_values:
        return torch.jit.trace(
            decoder_for_onnx, (input_ids, encoder_state, attention_mask, past_values)
        )
    else:
        return torch.jit.trace(
            decoder_for_onnx, (input_ids, encoder_state, attention_mask)
        )
...
class BARTGenerator(torch.nn.Module, GenerationMixin):
    def __init__(self, model):
        ...
        self.logits_processor = MinLengthLogitsProcessorTS(
            self.config.min_length, self.config.eos_token_id
        )
    ...
    def _trace_modules(self, model):
        ...
        self.encoder = _create_traced_encoder(
            model.get_encoder(), input_ids, attention_mask
        )
    ...
    def _init_sequence_length_for_generation(
        ...
        unfinished_sequences = (
            torch.zeros(input_ids.shape[0], dtype=torch.long, device=input_ids.device)
            + 1
        )
        sequence_lengths = (
            torch.zeros(input_ids.shape[0], dtype=torch.long, device=input_ids.device)
    ...
    def _decoder_forward(
        self, input_ids, encoder_output, attention_mask, past: List[torch.Tensor]
    ):
        ...
        lm_logits = F.linear(
            decoder_output, self.final_logits_weight, bias=self.final_logits_bias
        )
    ...
    def greedy_search(
        ...
        while cur_len < max_length:
            ...
            logits, past = self._decoder_forward(
                input_ids, encoder_output, attention_mask, past
            )
...
class BeamSearchScorerTS(torch.nn.Module):
    def __init__(self):
        ...
        self._beam_hyps: List[torch.Tensor] = [
            torch.zeros(2)
        ]  # placeholder for TorchScript compatibility
    ...
    def hypo_add(self, hyp: torch.Tensor, sum_logprobs: float, hypo_idx: int):
        ...
        if (
            hyps_count < self.num_beams
            or score > self._beam_hyps_worst_scores[hypo_idx]
        ):
            ...
            beam_idx = (
                torch.sum(self._beam_hyps_count[:hypo_idx])
                if hypo_idx != 0
            ...
            if hyps_count + 1 > self.num_beams:
                ...
                self._beam_hyps_worst_scores[hypo_idx] = min(
                    score, self._beam_hyps_worst_scores[hypo_idx]
                )
                self._beam_hyps_count[hypo_idx] = hyps_count + 1

    def hypo_is_done(
        self, hypo_idx: int, best_sum_logprobs: float, cur_len: int
    ) -> bool:
        """
    ...
    def finalize(
        ...
        sent_lengths = torch.zeros(
            batch_size * self.num_beam_hyps_to_keep, dtype=torch.long
        )
        best = []
        ...
        decoded = torch.zeros(
            batch_size * self.num_beam_hyps_to_keep, sent_max_len, dtype=torch.long
        )
...
class BARTBeamSearchGenerator(BARTGenerator):
    ...
    def beam_search(
        ...
        next_tokens = torch.zeros(
            (batch_size, num_beams), dtype=torch.long, device=input_ids.device
        )
    ...
    def forward(
        self, input_ids, attention_mask, num_beams, max_length, decoder_start_token_id
    ):
        pad_token_id = self.config.pad_token_id
        bos_token_id = self.config.bos_token_id
        eos_token_id = self.config.eos_token_id
