...
class FlaxEncoderDecoderMixin:
        ...
        **kwargs,
        ...
        with tempfile.TemporaryDirectory() as enc_tmpdir:
            with tempfile.TemporaryDirectory() as dec_tmpdir:
                ...
                enc_dec_model = (
                    FlaxSpeechEncoderDecoderModel.from_encoder_decoder_pretrained(
        ...
        self.assertNotEqual(
            config.add_adapter, enc_dec_model.config.encoder.add_adapter
        )
        self.assertNotEqual(
    ...
    def check_encoder_decoder_model_generate(
        self, inputs, config, decoder_config, **kwargs
    ):
        encoder_model, decoder_model = self.get_encoder_decoder_model(
            config, decoder_config
        )
        ...
        self.assertEqual(
            generated_sequences.shape, (inputs.shape[0],) + (decoder_config.max_length,)
        )
    ...
    def check_freeze_feature_encoder(
        ...
        **kwargs,
        ...
        def compute_loss(
            ...
            loss = cross_entropy(
                logits, onehot(labels=decoder_input_ids, num_classes=vocab_size)
            ).sum()
            return (loss, logits)
...
class FlaxWav2Vec2GPT2ModelTest(FlaxEncoderDecoderMixin, unittest.TestCase):
    ...
    def test_flaxwav2vec2gpt2_pt_flax_equivalence(self):
        pt_model = SpeechEncoderDecoderModel.from_pretrained(
        ...
        decoder_input_ids = ids_tensor(
            [batch_size, 4], fx_model.config.decoder.vocab_size
        )
        decoder_attention_mask = random_attention_mask([batch_size, 4])
        inputs_dict = {
            "inputs": input_values,
...
class FlaxWav2Vec2BertModelTest(FlaxEncoderDecoderMixin, unittest.TestCase):
    ...
    def test_flaxwav2vec2bert_pt_flax_equivalence(self):
        ...
        input_values = floats_tensor(
            [batch_size, 512], fx_model.config.encoder.vocab_size
        )
        attention_mask = random_attention_mask([batch_size, 512])
        decoder_input_ids = ids_tensor(
            [batch_size, 4], fx_model.config.decoder.vocab_size
