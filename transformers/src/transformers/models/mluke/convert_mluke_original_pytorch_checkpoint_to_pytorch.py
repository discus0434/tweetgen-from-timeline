...
def convert_luke_checkpoint(
    ...
    with open(
        os.path.join(pytorch_dump_folder_path, "tokenizer_config.json"), "r"
    ) as f:
        tokenizer_config = json.load(f)
    tokenizer_config["tokenizer_class"] = "MLukeTokenizer"
    with open(
        os.path.join(pytorch_dump_folder_path, "tokenizer_config.json"), "w"
    ) as f:
    ...
    model.tie_weights()
    assert (
        model.luke.embeddings.word_embeddings.weight == model.lm_head.decoder.weight
    ).all()
    assert (
        model.luke.entity_embeddings.entity_embeddings.weight
        == model.entity_predictions.decoder.weight
    ).all()

    # Check outputs
    tokenizer = MLukeTokenizer.from_pretrained(
        pytorch_dump_folder_path, task="entity_classification"
    )
    ...
    assert [e for e in multilingual_predicted_entities if e.startswith("en:")][
        0
    ] == "en:Japan"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        choices=["base", "large"],
        help="Size of the model to be converted.",
    )
