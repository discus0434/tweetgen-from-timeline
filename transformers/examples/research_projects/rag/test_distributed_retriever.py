...
if is_ray_available():
    ...
    from distributed_ray_retriever import (
...
class RagRetrieverTest(TestCase):
    ...
    def get_dummy_custom_hf_index_pytorch_retriever(
        self, init_retrieval: bool, from_disk: bool, port=12345
    ):
        dataset = self.get_dummy_dataset()
        config = RagConfig(
            retrieval_vector_size=self.retrieval_vector_size,
    ...
    def get_dummy_custom_hf_index_ray_retriever(
        self, init_retrieval: bool, from_disk: bool
    ):
        # Have to run in local mode because sys.path modifications at top of
        # file are not propogated to remote workers.
        # https://stackoverflow.com/questions/54338013/parallel-import-a-python-file-from-sibling-folder
    ...
    def distributed_retriever_check(
    ...
    def test_pytorch_distributed_retriever_retrieve(self):
        ...
        hidden_states = np.array(
            [np.ones(self.retrieval_vector_size), -np.ones(self.retrieval_vector_size)],
            dtype=np.float32,
        )

        self.distributed_retriever_check(
            self.get_dummy_pytorch_distributed_retriever(init_retrieval=True),
            hidden_states,
            n_docs,
        )
