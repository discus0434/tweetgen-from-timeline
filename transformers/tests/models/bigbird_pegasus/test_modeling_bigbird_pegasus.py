...
class BigBirdPegasusModelTest(
    ...
    all_generative_model_classes = (
        (BigBirdPegasusForConditionalGeneration,) if is_torch_available() else ()
    )
    ...
    def _check_batched_forward(self, attn_type, tolerance=1e-3):
        ...
        self.assertTrue(
            torch.allclose(
                logits_batched[0, -3:], logits_single_first[0, -3:], atol=tolerance
            )
        )
        ...
        self.assertTrue(
            torch.allclose(
                logits_batched[1, :3], logits_single_second[0, :3], atol=tolerance
            )
        )
...
class BigBirdPegasusStandaloneDecoderModelTest(
    ...
    all_generative_model_classes = (
        (BigBirdPegasusForCausalLM,) if is_torch_available() else ()
    )
    test_pruning = False
    is_encoder_decoder = False
