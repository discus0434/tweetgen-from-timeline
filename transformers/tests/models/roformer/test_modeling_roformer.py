...
class RoFormerModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    all_generative_model_classes = (
        (RoFormerForCausalLM,) if is_torch_available() else ()
    )

    def setUp(self):
        self.model_tester = RoFormerModelTester(self)
        self.config_tester = ConfigTester(
            self, config_class=RoFormerConfig, hidden_size=37
        )
...
class RoFormerSinusoidalPositionalEmbeddingTest(unittest.TestCase):
    ...
    def test_basic(self):
        ...
        emb1 = RoFormerSinusoidalPositionalEmbedding(
            num_positions=6, embedding_dim=6
        ).to(torch_device)
...
class RoFormerSelfAttentionRotaryPositionEmbeddingTest(unittest.TestCase):
    ...
    def test_apply_rotary_position_embeddings(self):
        ...
        query_layer = (
            torch.arange(
                2 * 12 * 16 * 64, dtype=torch.float, device=torch_device
            ).reshape(2, 12, 16, 64)
            / 100
        ).to(torch_device)
        key_layer = (
        ...
        embed_positions = RoFormerSinusoidalPositionalEmbedding(
            num_positions=32, embedding_dim=64
        ).to(torch_device)
        ...
        self.assertTrue(
            torch.allclose(
                query_layer[0, 0, :6, :8], desired_query_layer, atol=self.tolerance
            ),
            msg=f"\nexp:\n{desired_query_layer}\ngot:\n{query_layer}\n",
        )
        self.assertTrue(
            torch.allclose(
                key_layer[0, 0, :6, :8], desired_key_layer, atol=self.tolerance
            ),
            msg=f"\nexp:\n{desired_key_layer}\ngot:\n{key_layer}\n",
        )
