                ...
                tf.config.set_logical_device_configuration(
                    ...
                    [
                        tf.config.LogicalDeviceConfiguration(
                            memory_limit=_tf_gpu_memory_limit
                        )
                    ],
                )
...
class TFModelTesterMixin:
        ...
        if return_labels:
            if model_class in get_values(TF_MODEL_FOR_MULTIPLE_CHOICE_MAPPING):
                ...
                *get_values(TF_MODEL_FOR_SPEECH_SEQ_2_SEQ_MAPPING),
                ...
                inputs_dict["labels"] = tf.zeros(
                    (self.model_tester.batch_size, height, width), dtype=tf.int32
                )
        ...
        for model_class in self.all_model_classes:
            ...
            if model.config.is_encoder_decoder:
                ...
                expected_arg_names.extend(
                    ...
                    if "decoder_position_ids" in arg_names
                    else []
                )
                expected_arg_names.extend(
    ...
    def check_pt_tf_outputs(
        ...
        if isinstance(tf_outputs, ModelOutput):
            ...
            tf_outputs, pt_outputs = self._postprocessing_to_ignore_test_cases(
                tf_outputs, pt_outputs, model_class
            )
    ...
    def test_pt_tf_model_equivalence(self):
        ...
        for model_class in self.all_model_classes:
            ...
            if set(tf_inputs_dict_with_labels.keys()).symmetric_difference(
                tf_inputs_dict.keys()
            ):
    ...
    def test_compile_tf_model(self):
        ...
        for model_class in self.all_model_classes:
            if model_class.__name__ in [
                ...
                inputs = {
                    ...
                    "input_features": tf.keras.Input(
                        batch_shape=(
                            2,
                            max_input,
                            self.model_tester.input_feat_per_channel
                            * self.model_tester.input_channels,
                        ),
                        name="input_features",
                        dtype="float32",
                    ),
                ...
                inputs = {
                    ...
                    "input_ids": tf.keras.Input(
                        batch_shape=(2, max_input), name="input_ids", dtype="int32"
                    ),
                }
            # `pixel_values` implies that the input is an image
                ...
                inputs = {
                    "input_ids": tf.keras.Input(
                        batch_shape=(3, max_input), name="input_ids", dtype="int32"
                    ),
                ...
                inputs = tf.keras.Input(
                    batch_shape=(4, 2, max_input), name="input_ids", dtype="int32"
                )
            else:
                inputs = tf.keras.Input(
                    batch_shape=(2, max_input), name="input_ids", dtype="int32"
    ...
    def test_attention_outputs(self):
        ...
        decoder_key_length = getattr(
            self.model_tester, "key_length", decoder_seq_length
        )
        ...
        for model_class in self.all_model_classes:
            ...
            def check_attentions_validity(attentions):
                ...
                for t in attentions:
                    self.assertLess(
                        (
                            tf.math.reduce_sum(tf.cast(tf.math.is_nan(t), tf.float32))
                        ).numpy(),
                        (tf.size(t) / 4).numpy(),
                ...
                self.assertAlmostEqual(
                    tf.math.reduce_sum(attentions[0][..., 0, :, :]).numpy(), 0.0
                )
                self.assertNotEqual(
                    tf.math.reduce_sum(attentions[0][..., -1, :, :]).numpy(), 0.0
                )
                if (
                    len(attentions) > 2
                ):  # encoder-decodere models have only 2 layers in each modules
                    self.assertNotEqual(
                        tf.math.reduce_sum(attentions[1][..., 0, :, :]).numpy(), 0.0
                    )
                self.assertAlmostEqual(
                    tf.math.reduce_sum(attentions[-1][..., -2, :, :]).numpy(), 0.0
                )
                self.assertNotEqual(
                    tf.math.reduce_sum(attentions[-1][..., -1, :, :]).numpy(), 0.0
                )
            ...
            for size in [config.vocab_size - 10, config.vocab_size + 10, None]:
                ...
                if old_bias is not None and new_bias is not None:
                    for old_weight, new_weight in zip(
                        old_bias.values(), new_bias.values()
                    ):
                ...
                if (
                    ...
                    self.assertEqual(
                        new_output_embeddings.shape[1], old_output_embeddings.shape[1]
                    )
    ...
    def test_lm_head_model_random_no_beam_search_generate(self):
        ...
        for model_class in self.all_generative_model_classes:
            ...
            self._check_generated_ids(
                model.generate(input_ids, do_sample=True, num_return_sequences=2)
            )
    ...
    def test_lm_head_model_random_beam_search_generate(self):
        ...
        for model_class in self.all_generative_model_classes:
            ...
            if config.bos_token_id is None:
                ...
                self._check_generated_ids(
                    model.generate(input_ids, do_sample=True, num_beams=2)
                )
            else:
                # num_return_sequences = 1
                self._check_generated_ids(
                    model.generate(do_sample=True, max_length=5, num_beams=2)
                )

            with self.assertRaises(ValueError):
                # generating more sequences than having beams leads is not possible
                model.generate(
                    input_ids, do_sample=False, num_return_sequences=3, num_beams=2
                )
            ...
            self._check_generated_ids(
                model.generate(
                    input_ids, do_sample=False, num_beams=2, num_return_sequences=2
                )
            )
        ...
        for model_class in self.all_generative_model_classes:
            ...
            if model.config.is_encoder_decoder:
                self.assertIsInstance(
                    output_beam_search, TFBeamSearchEncoderDecoderOutput
                )
                self.assertIsInstance(
                    output_beam_sample, TFBeamSampleEncoderDecoderOutput
                )
            else:
    ...
    def test_loss_computation(self):
        ...
        for model_class in self.all_model_classes:
            ...
            if getattr(model, "hf_compute_loss", None):
                ...
                self.assertTrue(
                    loss.shape.as_list() == expected_loss_size
                    or loss.shape.as_list() == [1]
                )
    ...
    def test_keras_fit(self):
        ...
        for model_class in self.all_model_classes:
            ...
            if getattr(model, "hf_compute_loss", None):
                ...
                model(
                    model.dummy_inputs
                )  # Build the model so we can get some constant weights
                model_weights = model.get_weights()

                # Run eagerly to save some expensive compilation times
                model.compile(
                    optimizer=tf.keras.optimizers.SGD(0.0),
                    run_eagerly=True,
                    metrics=metrics,
                )
                ...
                accuracy1 = {
                    key: val[0]
                    for key, val in history1.history.items()
                ...
                accuracy2 = {
                    key: val[0]
                    for key, val in history2.history.items()
                    if key.endswith("accuracy")
                }
                ...
                for key in history1.history.keys():
                    if not key.startswith("val_"):
                        self.assertTrue(
                            "val_" + key in history1.history.keys(),
                            "Outputs differ in train/test step!",
                        )
                if metrics:
                    self.assertTrue(
                        len(accuracy1) == len(accuracy2) > 0, "Missing metrics!"
                    )
                ...
                accuracy3 = {
                    key: val[0]
                    for key, val in history3.history.items()
                    if key.endswith("accuracy")
                }
                ...
                if metrics:
                    self.assertTrue(
                        len(accuracy1) == len(accuracy3) > 0, "Missing metrics!"
                    )
        ...
        for model_class in self.all_model_classes:
            ...
            if not any(
                [
                    ...
                    for tensor in prepared_for_class.values()
            ...
            prepared_for_class = {
                key: tf.cast(tensor, tf.int64)
                if isinstance(tensor, tf.Tensor) and tensor.dtype.is_integer
                else tensor
            ...
            model(
                **prepared_for_class
            )  # No assertion, we're just checking this doesn't throw an error
    ...
    def test_generate_with_headmasking(self):
        ...
        for model_class in self.all_generative_model_classes:
            ...
            head_masking = {
                ...
                "decoder_head_mask": tf.zeros(
                    (config.decoder_layers, config.decoder_attention_heads)
                ),
                "cross_attn_head_mask": tf.zeros(
                    (config.decoder_layers, config.decoder_attention_heads)
                ),
            }
            ...
            for attn_name, (name, mask) in zip(attention_names, head_masking.items()):
                ...
                self.assertEqual(
                    sum([tf.reduce_sum(w).numpy() for w in attn_weights]), 0.0
                )
    ...
    def test_dataset_conversion(self):
        ...
        for model_class in self.all_model_classes:
            model = model_class(config)
            tf_inputs_dict = self._prepare_for_class(
                inputs_dict, model_class, return_labels=False
            )
            tf_inputs_dict = {
                key: val
            ...
            tf_inputs_dict["extra_unwanted_column"] = list(tf_inputs_dict.values())[
                0
            ]  # Use a random other tensor
            input_dataset = Dataset.from_dict(tf_inputs_dict)
            tf_dataset = model.prepare_tf_dataset(
                input_dataset,
                batch_size=len(input_dataset),
            ...
            if isinstance(test_batch, tf.Tensor):
                self.assertEqual(
                    len(test_batch), len(input_dataset)
                )  # Assert we didn't lose any data
            else:
            ...
            if "labels" in inspect.signature(model_class.call).parameters.keys():
                ...
                self.assertGreater(
                    len(test_batch_labels), 0
                )  # Assert the labels are present
                feature_columns = (
                    1 if isinstance(test_batch, tf.Tensor) else len(test_batch)
                )
                label_columns = (
                    1
                    if isinstance(test_batch_labels, tf.Tensor)
                    else len(test_batch_labels)
                )
                # Assert we discarded the unwanted extra column but kept everything else
                self.assertEqual(
                    feature_columns + label_columns, len(input_dataset.features) - 1
                )
    ...
    def _generate_random_bad_tokens(self, num_bad_tokens, model):
        ...
        while len(bad_tokens) < num_bad_tokens:
            token = tf.squeeze(
                ids_tensor((1, 1), self.model_tester.vocab_size), 0
            ).numpy()[0]
...
def random_attention_mask(shape, rng=None, name=None, dtype=None):
    ...
    attn_mask = tf.concat(
        [attn_mask[:, :-1], tf.ones_like(attn_mask[:, -1:], dtype=dtype)], axis=-1
    )
    return attn_mask
...
class UtilsFunctionsTest(unittest.TestCase):
    ...
    def test_top_k_top_p_filtering(self):
        ...
        non_inf_idx = tf.cast(
            tf.where(
                tf.not_equal(output, tf.constant(-float("inf"), dtype=tf.float32))
            ),
            dtype=tf.int32,
        )
    ...
    def test_unpack_inputs(self):
        ...
        output = dummy_model.call(
            input_ids=input_ids, past=past, output_attentions=False, return_dict=True
        )
        tf.debugging.assert_equal(output[0], input_ids)
        tf.debugging.assert_equal(output[1], past)
        self.assertFalse(output[2])
    ...
    def test_checkpoint_sharding_local_from_pt(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            _ = Repository(
                local_dir=tmp_dir,
                clone_from="hf-internal-testing/tiny-random-bert-sharded",
            )
            ...
            ref_model = TFBertModel.from_pretrained(
    ...
    def test_generate_tf_function_export(self):
        test_model = TFAutoModelForSeq2SeqLM.from_pretrained(
...
class TFModelPushToHubTester(unittest.TestCase):
    ...
    def test_push_to_hub(self):
        ...
        self.assertIn(
            "Uploading README.md to __DUMMY_TRANSFORMERS_USER__/test-model-tf", cl.out
        )
    ...
    def test_push_to_hub_in_organization(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            model.save_pretrained(
                ...
                push_to_hub=True,
                use_auth_token=self._token,
                repo_id="valid_org/test-model-tf-org",
            )
