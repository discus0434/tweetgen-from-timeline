...
class DebertaModelTester(object):
    ...
    def create_and_check_deberta_for_sequence_classification(
        ...
        self.parent.assertListEqual(
            list(result.logits.size()), [self.batch_size, self.num_labels]
        )
...
class DebertaModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(
            self, config_class=DebertaConfig, hidden_size=37
        )
    ...
    def test_for_sequence_classification(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_deberta_for_sequence_classification(
            *config_and_inputs
        )
    ...
    def test_for_question_answering(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_deberta_for_question_answering(
            *config_and_inputs
        )

    def test_for_token_classification(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_deberta_for_token_classification(
            *config_and_inputs
        )
...
class DebertaModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_no_head(self):
        ...
        expected_slice = torch.tensor(
            [
                [
                    [-0.5986, -0.8055, -0.8462],
                    [1.4484, -0.9348, -0.8059],
                    [0.3123, 0.0032, -1.4131],
                ]
            ]
        )
        self.assertTrue(
