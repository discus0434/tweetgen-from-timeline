    ...
    def interpolate_pos_encoding(
        ...
        patch_pos_embed = nn.functional.interpolate(
            patch_pos_embed.reshape(
                1, int(math.sqrt(num_pos)), int(math.sqrt(num_pos)), dim
            ).permute(0, 3, 1, 2),
            scale_factor=(
                num_h_patches / math.sqrt(num_pos),
                num_w_patches / math.sqrt(num_pos),
            ),
            mode="bicubic",
            align_corners=False,
        )
...
class FlavaPreTrainedModel(PreTrainedModel):
    ...
    def _set_gradient_checkpointing(
        self, module: FlavaEncoder, value: bool = False
    ) -> None:
    ...
    def forward(
        ...
        if (
            ...
            multimodal_output = self.multimodal_model(
                multimodal_input, return_dict=return_dict
            )
...
class FlavaForPreTraining(FlavaPreTrainedModel):
    ...
    def forward(
        ...
        total_loss = (
            mim_loss
        ) = mlm_loss = mmm_text_loss = mmm_image_loss = gc_loss = itm_loss = None
