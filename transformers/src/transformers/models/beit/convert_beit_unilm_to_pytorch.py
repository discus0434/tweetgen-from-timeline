...
def read_in_q_k_v(state_dict, config, has_lm_head=False, is_semantic=False):
    for i in range(config.num_hidden_layers):
        ...
        state_dict[
            f"beit.encoder.layer.{i}.attention.attention.query.weight"
        ] = in_proj_weight[: config.hidden_size, :]
        state_dict[f"beit.encoder.layer.{i}.attention.attention.query.bias"] = q_bias
        state_dict[
            f"beit.encoder.layer.{i}.attention.attention.key.weight"
        ] = in_proj_weight[config.hidden_size : config.hidden_size * 2, :]
        state_dict[
            f"beit.encoder.layer.{i}.attention.attention.value.weight"
        ] = in_proj_weight[-config.hidden_size :, :]
...
def convert_beit_checkpoint(checkpoint_url, pytorch_dump_folder_path):
    ...
    state_dict = (
        state_dict["model"]
        if "ade20k" not in checkpoint_url
        else state_dict["state_dict"]
    )

    rename_keys = create_rename_keys(
        config, has_lm_head=has_lm_head, is_semantic=is_semantic
    )
    ...
    if is_semantic:
        feature_extractor = BeitFeatureExtractor(
            size=config.image_size, do_center_crop=False
        )
        ds = load_dataset("hf-internal-testing/fixtures_ade20k", split="test")
        image = Image.open(ds[0]["file"])
    else:
        feature_extractor = BeitFeatureExtractor(
            size=config.image_size, resample=Image.BILINEAR, do_center_crop=False
        )
        image = prepare_img()
    ...
    if checkpoint_url[:-4].endswith("beit_base_patch16_224_pt22k"):
        ...
        expected_logits = torch.tensor(
            [
                [
                    [-4.3305, -2.3049, -3.0161],
                    [-2.9591, -1.5305, -2.2251],
                    [-3.4198, -1.8004, -2.9062],
                ],
                [
                    [-5.8922, -3.7435, -4.3978],
                    [-4.2063, -2.7872, -3.4755],
                    [-4.2791, -3.1874, -4.1681],
                ],
                [
                    [0.9895, 4.3467, 4.7663],
                    [4.2476, 5.6830, 6.1518],
    ...
    if not has_lm_head:
        if is_semantic:
            ...
            ), "Predicted class index not as expected"
