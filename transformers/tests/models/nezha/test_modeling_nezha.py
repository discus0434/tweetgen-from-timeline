...
if is_torch_available():
    ...
    from transformers.models.nezha.modeling_nezha import (
...
class NezhaModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(
            self, config_class=NezhaConfig, hidden_size=37
        )
        ...
        for model_class in self.all_model_classes:
            ...
            with tempfile.TemporaryDirectory() as tmp:
                ...
                loaded = torch.jit.load(
                    os.path.join(tmp, "bert.pt"), map_location=torch_device
                )
                loaded(
...
class NezhaModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_nezha_masked_lm(self):
        ...
        expected_slice = torch.tensor(
            [
                [-2.7939, -1.7902, -2.2189],
                [-2.8585, -1.8908, -2.3723],
                [-2.6499, -1.7750, -2.2558],
            ]
        )
