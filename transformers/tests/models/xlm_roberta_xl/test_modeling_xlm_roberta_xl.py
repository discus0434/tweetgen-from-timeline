...
class XLMRobertaXLModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    all_generative_model_classes = (
        (XLMRobertaXLForCausalLM,) if is_torch_available() else ()
    )

    def setUp(self):
        self.model_tester = XLMRobertaXLModelTester(self)
        self.config_tester = ConfigTester(
            self, config_class=XLMRobertaXLConfig, hidden_size=37
        )
...
class XLMRobertaModelXLIntegrationTest(unittest.TestCase):
    ...
    def test_xlm_roberta_xl(self):
        model = XLMRobertaXLModel.from_pretrained("facebook/xlm-roberta-xl").to(
            torch_device
        )
    ...
    def test_xlm_roberta_xxl(self):
        model = XLMRobertaXLModel.from_pretrained("facebook/xlm-roberta-xxl").to(
            torch_device
        )
        input_ids = torch.tensor(
            [[0, 581, 10269, 83, 99942, 136, 60742, 23, 70, 80583, 18276, 2]],
