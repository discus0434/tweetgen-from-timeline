...
class QDQBertSelfOutput(nn.Module):
    def __init__(self, config):
        ...
        self.add_local_input_quantizer = TensorQuantizer(
            quant_nn.QuantLinear.default_quant_desc_input
        )
        self.add_residual_input_quantizer = TensorQuantizer(
            quant_nn.QuantLinear.default_quant_desc_input
        )
...
class QDQBertLMHeadModel(QDQBertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if not config.is_decoder:
            logger.warning(
                "If you want to use `QDQBertLMHeadModel` as a standalone, add `is_decoder=True.`"
            )
...
@add_start_docstrings(
    """QDQBERT Model with a `language modeling` head on top.""", QDQBERT_START_DOCSTRING
)
