...
class TFLongformerModelTester:
    ...
    def create_and_check_attention_mask_determinism(
        ...
        tf.debugging.assert_near(
            output_with_mask[0, 0, :5], output_without_mask[0, 0, :5], rtol=1e-4
        )
    ...
    def create_and_check_model(
        ...
        self.parent.assertListEqual(
            shape_list(result.pooler_output), [self.batch_size, self.hidden_size]
        )
    ...
    def create_and_check_for_question_answering(
        ...
        self.parent.assertListEqual(
            shape_list(result.start_logits), [self.batch_size, self.seq_length]
        )
        self.parent.assertListEqual(
            shape_list(result.end_logits), [self.batch_size, self.seq_length]
        )
    ...
    def create_and_check_for_sequence_classification(
        ...
        self.parent.assertListEqual(
            shape_list(output), [self.batch_size, self.num_labels]
        )
    ...
    def create_and_check_for_token_classification(
        ...
        self.parent.assertListEqual(
            shape_list(output), [self.batch_size, self.seq_length, self.num_labels]
        )
    ...
    def create_and_check_for_multiple_choice(
        ...
        self.parent.assertListEqual(
            list(output.shape), [self.batch_size, self.num_choices]
        )
    ...
    def prepare_config_and_inputs_for_question_answering(self):
        ...
        input_ids = tf.concat(
            [input_ids[:, :-3], tf.ones_like(input_ids)[:, -3:] * config.sep_token_id],
            axis=-1,
        )
...
class TFLongformerModelTest(TFModelTesterMixin, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(
            self, config_class=LongformerConfig, hidden_size=37
        )
...
class TFLongformerModelIntegrationTest(unittest.TestCase):
    ...
    def test_diagonalize(self):
        ...
        tf.debugging.assert_near(
            padded_hidden_states[0, 0, 0, :4], chunked_hidden_states[0, 0, 0], rtol=1e-3
        )
        tf.debugging.assert_near(
    ...
    def test_pad_and_transpose_last_two_dims(self):
        ...
        hidden_states = TFLongformerSelfAttention._chunk(
            hidden_states, window_overlap=2
        )
        padded_hidden_states = (
            TFLongformerSelfAttention._pad_and_transpose_last_two_dims(
    ...
    def test_mask_invalid_locations(self):
        ...
        self.assertTrue(
            tf.math.reduce_sum(tf.cast(tf.math.is_inf(hid_states_1), tf.dtypes.int32))
            == 8
        )
        self.assertTrue(
            tf.math.reduce_sum(tf.cast(tf.math.is_inf(hid_states_2), tf.dtypes.int32))
            == 24
        )
        self.assertTrue(
            tf.math.reduce_sum(tf.cast(tf.math.is_inf(hid_states_3), tf.dtypes.int32))
            == 24
        )
        self.assertTrue(
            tf.math.reduce_sum(tf.cast(tf.math.is_inf(hid_states_4), tf.dtypes.int32))
            == 12
        )
    ...
    def test_chunk(self):
        ...
        tf.debugging.assert_near(
            chunked_hidden_states[0, 0, :, 0], expected_slice_along_chunk, rtol=1e-3
        )
    ...
    def test_layer_global_attn(self):
        ...
        tf.debugging.assert_near(
            output_hidden_states[0, 2], expected_slice_0, rtol=1e-3
        )
        tf.debugging.assert_near(
            output_hidden_states[1, -2], expected_slice_1, rtol=1e-3
        )
    ...
    def test_layer_attn_probs(self):
        ...
        self.assertTrue(
            (tf.math.abs(tf.math.reduce_sum(global_attentions, axis=-1) - 1) < 1e-6)
            .numpy()
            .tolist()
        )
        ...
        tf.debugging.assert_near(
            ...
            tf.convert_to_tensor(
                [0.2500, 0.2500, 0.2500, 0.2500], dtype=tf.dtypes.float32
            ),
            rtol=1e-3,
        )
        tf.debugging.assert_near(
            ...
            tf.convert_to_tensor(
                [0.2497, 0.2500, 0.2499, 0.2504], dtype=tf.dtypes.float32
            ),
            rtol=1e-3,
        )
    ...
    def test_inference_no_head(self):
        ...
        input_ids = tf.convert_to_tensor(
            [[0, 20920, 232, 328, 1437, 2]], dtype=tf.dtypes.int32
        )
        ...
        tf.debugging.assert_near(
            output_without_mask[0, 0, -5:], expected_output_slice, rtol=1e-3
        )
    ...
    def test_inference_no_head_long(self):
        ...
        input_ids = tf.convert_to_tensor(
            [[0] + [20920, 232, 328, 1437] * 1000 + [2]], dtype=tf.dtypes.int32
        )
        ...
        tf.debugging.assert_near(
            tf.reduce_mean(output), expected_output_mean, rtol=1e-4
        )
    ...
    def test_inference_masked_lm_long(self):
        ...
        tf.debugging.assert_near(
            tf.reduce_sum(prediction_scores), expected_prediction_scores_sum, rtol=1e-4
        )
        tf.debugging.assert_near(
            tf.reduce_mean(prediction_scores),
