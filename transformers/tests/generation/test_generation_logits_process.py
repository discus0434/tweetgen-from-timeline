...
class LogitsProcessorTest(unittest.TestCase):
    ...
    def test_temperature_dist_warper(self):
        ...
        warped_prob_sharp = nn.functional.softmax(
            temp_dist_warper_sharper(input_ids, scores.clone()), dim=-1
        )
        warped_prob_smooth = nn.functional.softmax(
            temp_dist_warper_smoother(input_ids, scores.clone()), dim=-1
        )

        # uniform distribution stays uniform
        self.assertTrue(torch.allclose(probs[0, :], warped_prob_sharp[0, :], atol=1e-3))
        self.assertTrue(
            torch.allclose(probs[0, :], warped_prob_smooth[0, :], atol=1e-3)
        )
    ...
    def test_top_p_dist_warper(self):
        ...
        self.assertTrue(
            torch.allclose(filtered_dist, EXPECTED_FILTERED_DIST, atol=1e-3)
        )
        ...
        self.assertTrue(
            torch.allclose(
                ...
                torch.tensor(
                    [
                        [0.0, 0.7, 0.8, 0.0],
                        [0.1, torch.finfo(scores.dtype).max, 0.3, float("-inf")],
                    ],
                    device=torch_device,
                ),
                atol=1e-6,
    ...
    def test_exponential_decay_length_penalty(self):
        ...
        self.assertTrue(
            torch.gt(
                scores_after_start[penalty_start + 1 :, eos_token_id],
                scores[penalty_start + 1 :, eos_token_id],
            ).all()
        )
