...
class RagTestMixin:
    ...
    def get_retriever(self, config):
        ...
        tokenizer = (
            self.bart_tokenizer
            if config.generator.model_type == "bart"
            else self.t5_tokenizer
        )
        with patch(
    ...
    def check_model_with_retriever(
        ...
        **kwargs,
        ...
        for model_class in self.all_model_classes:
            model = model_class(config, retriever=self.get_retriever(config)).to(
                torch_device
            )
            model.eval()
    ...
    def check_model_with_end2end_retriever(
        ...
        **kwargs,
        ...
        )  # setting the ctx_encoder_tokenizer.
        ...
        for model_class in [RagTokenForGeneration, RagSequenceForGeneration]:
            ...
            model.set_context_encoder_for_training(
                dpr_context_encoder
            )  # set the context_encoder for training
...
class RagModelIntegrationTests(unittest.TestCase):
    ...
    def test_rag_token_generate_beam(self):
        ...
        output_text_1 = rag_decoder_tokenizer.decode(
            output_ids[0], skip_special_tokens=True
        )
        output_text_2 = rag_decoder_tokenizer.decode(
