...
class FlaxBlenderbotModelTest(
    ...
    all_generative_model_classes = (
        (FlaxBlenderbotForConditionalGeneration,) if is_flax_available() else ()
    )
    ...
    def test_generation_from_short_input_same_as_parlai_3B(self):
        FASTER_GEN_KWARGS = dict(
            num_beams=1, early_stopping=True, min_length=15, max_length=25
        )
        TOK_DECODE_KW = dict(
        ...
        model = FlaxBlenderbotForConditionalGeneration.from_pretrained(
            "facebook/blenderbot-3B", from_pt=True
        )
