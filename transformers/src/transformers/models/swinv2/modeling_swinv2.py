...
class Swinv2SelfAttention(nn.Module):
    ...
    def forward(
        ...
        # cosine attention
        attention_scores = F.normalize(query_layer, dim=-1) @ F.normalize(
            key_layer, dim=-1
        ).transpose(-2, -1)
...
class Swinv2Layer(nn.Module):
    def __init__(
        ...
        self.drop_path = (
            Swinv2DropPath(config.drop_path_rate)
    ...
    def set_shift_and_window_size(self, input_resolution):
        ...
        self.window_size = (
            input_resolution[0]
            if input_resolution[0] <= target_window_size[0]
            else target_window_size[0]
        )
        self.shift_size = (
            0
            if input_resolution
